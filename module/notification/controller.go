...
//Update : get list of email that can receive update from an email address
func (c *Controller) Update(ctx *gin.Context) {
	...
	result, err := c.notificationService.Update(&request)
	if err != nil {
		ctx.JSON(http.StatusBadRequest, gin.H{"success": result, "errors": err.Error()})
		return
	}

	ctx.JSON(http.StatusOK, gin.H{"success": true, "recipients": result})
}
